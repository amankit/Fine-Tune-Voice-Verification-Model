...
def save_checkpoint(state,filename='saved_models_cross_entropy/best_checkpoint.pth'):
...
def main():
    ...
    train_dataset = FBanksCrossEntropyDataset('/home/ankit/Downloads/LibriSpeech/features_train')
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, **kwargs)

    test_dataset = FBanksCrossEntropyDataset('/home/ankit/Downloads/LibriSpeech/features_test')
    ...
    old_test_accuracy = 0
    if load_model==True:
        load_checkpoint(torch.load('saved_models_cross_entropy/best_checkpoint.pth'))
    for epoch in range(0, 20):
        ...
        if test_accuracy>old_test_accuracy:
            print('OLD TEST ACCURACY: ',old_test_accuracy)
